...
int execute_line(char *line){
    ...
    if(check_internal(troceada) == 0){
        ...
        if(proceso > 0){
            ...
            int status;
            pid_t terminated_pid = waitpid(proceso, &status, 0);
            if (terminated_pid == -1) {
                fprintf(stderr, "\033[1;31m[ERROR EN WAITPID]\033[0m\n"); // Error en rojo
            } else {
                #if DEBUGN3
                printf("\033[1;30m[Proceso hijo con PID %d finalizado con estado %d]\033[0m\n", terminated_pid, WEXITSTATUS(status)); // Seguimiento en gris
                #endif
            }
